...
function FeatureCard({ index }: { index: number }) {
  ...
  return (
    <Link
      ...
      onClick={(e) => { e.preventDefault(); setLocation(`/article/${article.slug}`); window.scrollTo({ top: 0, behavior: "smooth" }); }}
    >
      <img src={article.image} alt="" />
      <div className="feature-shade" />
      <div className="feature-content">
        <span className="micro-label">{article.eyebrow}</span>
        <h3>{article.title}</h3>
        <span className="feature-read">Open dossier <ArrowUpRight size={16} /></span>
      </div>
    </Link>
  );
}
...
function RouteTile({ icon: Icon, code, title, body, href, tone }: { icon: typeof Compass; code: string; title: string; body: string; href: string; tone: string }) {
  ...
  return (
    <Link
      href={href}
      className={`route-tile route-${tone}`}
      onClick={(e) => { e.preventDefault(); setLocation(href); window.scrollTo({ top: 0, behavior: "smooth" }); }}
    >
      <Icon size={21} />
      <span className="micro-label">{code}</span>
      <h3>{title}</h3>
      <p>{body}</p>
      <span className="route-tile-arrow"><ArrowUpRight size={17} /></span>
    </Link>
  );
}
...
export default function Home() {
  ...
  const navigateTo = (href: string) => (event: MouseEvent<HTMLAnchorElement | HTMLButtonElement>) => {
    event.preventDefault();
    setLocation(href);
    window.scrollTo({ top: 0, behavior: reduceMotion ? "auto" : "smooth" });
  };

  const pullToDesk = (event: MouseEvent<HTMLAnchorElement | HTMLButtonElement>) => {
    event.preventDefault();
    const desk = document.getElementById("om-favourites");
    if (!desk) return;
    const yOffset = -76;
    const y = desk.getBoundingClientRect().top + window.pageYOffset + yOffset;
  ...
  return (
    <main>
      <section className="hero-section om-hero">
        ...
        <Link href="/archive" onClick={navigateTo("/archive")} className="hero-field-note" title="Explore field notes"><span>FIELD NOTE / 𓂀</span><strong>Begin with the mark.<br />Not the myth.</strong><i>OMN · 2026</i></Link>
        <Link href="/om-dashboard" onClick={navigateTo("/om-dashboard")} className="om-hero-seal" title="Open Curator Station"><span>OM</span><i>FIELD<br />STATION</i></Link>
        ...
        <motion.div className="hero-copy" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}>
          ...
          <div className="hero-actions">
            <Link href="/archive" onClick={navigateTo("/archive")} className="ink-button">Browse all records <ArrowUpRight size={18} /></Link>
            <button type="button" onClick={pullToDesk} className="text-button"><ArrowDown size={17} /> See Om’s starting shelf</button>
            <Link href="/about" onClick={navigateTo("/about")} className="hero-method-link">New here? Read the field guide</Link>
          </div>
        </motion.div>
        <div className="hero-attribution">CREATED &amp; CURATED BY<br /><strong>OM NANDURKAR</strong></div>
        <div className="keeper-access"><Link href="/om-dashboard?desk=cases" onClick={navigateTo("/om-dashboard?desk=cases")}><KeyRound size={14} /> {keeperLabel}</Link></div>
      </section>
      ...
      <section className="om-station-strip">
        ...
        <Link href="/investigations" onClick={navigateTo("/investigations")} className="station-metric"><b>13</b><span>ANOMALY<br />FILES</span></Link>
        <Link href="/archive" onClick={navigateTo("/archive")} className="station-metric"><b>03</b><span>LONG-READ<br />RECORDS</span></Link>
        <Link href="/index" onClick={navigateTo("/index")} className="station-metric"><b>05</b><span>EVIDENCE<br />MODES</span></Link>
        <Link href="/om-dashboard?desk=cases" onClick={navigateTo("/om-dashboard?desk=cases")} className="station-metric station-status"><span className="status-dot" /> <small>OM’S DESK<br />OPEN</small></Link>
      </section>

      <section id="om-favourites" ref={favouritesRef} className="om-favourites-section">
        <div className="favourites-heading"><span className="micro-label">CURATOR’S DRAWER / OMN</span><h2>Om’s <em>favourites.</em></h2><p>The strange records that keep returning to the top of the desk — selected by Om Nandurkar for readers who like their history with a little static in the signal.</p><Link href="/field-folio" onClick={navigateTo("/field-folio")} className="favourites-folio-link">Turn Om’s field folio <ArrowUpRight size={14} /></Link></div>
        <div className="favourites-board"><div aria-hidden="true" className="favourites-parallax-trace" />
          <Link href="/field-folio" onClick={navigateTo("/field-folio")} className="favourites-specimen"><motion.img className="home-parallax-image" style={reduceMotion ? {} : { y: specimenY }} src="/assets/om-specimen-field-station.webp" alt="Om’s archaeological and natural-history specimen board" /><div className="favourites-specimen-copy"><span className="micro-label">SPECIMEN BOARD / OM-01</span><h3>Things Om keeps close.</h3><p>Patterns, protective eyes, desert life, and small fragments that make an archive feel awake.</p></div><span className="favourites-pin" /></Link>
          <Link href="/investigations" onClick={navigateTo("/investigations")} className="favourites-sticker-sheet"><motion.img className="home-parallax-image" style={reduceMotion ? {} : { y: stickerY }} src="/assets/om-field-station-stickers.webp" alt="Om’s six field-station research motifs" /><span>OM’S FIELD TOOLS<br />06 MOTIFS / ACTIVE</span></Link>
          <Link href={`/article/${articles[0].slug}`} onClick={navigateTo(`/article/${articles[0].slug}`)} className="favourite-folio favourite-folio-one"><span className="folio-tab">01 / STONE</span><h3>The Sphinx waterline</h3><p>Weathering, chronology, and a question carved into limestone.</p><span className="folio-link-text">Read the record <ArrowUpRight size={15} /></span></Link>
          <Link href="/investigations" onClick={navigateTo("/investigations")} className="favourite-folio favourite-folio-two"><span className="folio-tab">02 / SIGNAL</span><h3>God-born bodies</h3><p>Mutation myths and superhuman archetypes inside the Anomaly Index.</p><span className="folio-link-text">Open the file <ArrowUpRight size={15} /></span></Link>
      ...
      <section id="field-notes" className="field-notes-section">
        ...
        <div className="manifesto-layout"><div className="manifesto-title"><span className="scribble-mark">✦</span><h2>Unseal the<br /><em>evidence.</em></h2></div><div className="manifesto-body"><p className="lead-copy">Every record begins with a fragment: a ridge in limestone, a star above a pyramid, a symbol that refuses a simple translation.</p><p>Theorem of Kemet makes room for the thrill of an unanswered question — while keeping archaeology, source trails, and counterarguments in view. Consider this a desk for slow curiosity.</p><Link href="/about" onClick={navigateTo("/about")} className="underlined-link">Read the field guide <ArrowUpRight size={15} /></Link></div></div>
        <div className="principles-strip">
          <Link href="/investigations" onClick={navigateTo("/investigations")}><Compass /><span>FOLLOW THE CLUE</span></Link>
          <Link href="/archive" onClick={navigateTo("/archive")}><ScrollText /><span>READ THE CONTEXT</span></Link>
          <Link href="/about" onClick={navigateTo("/about")}><Feather /><span>HOLD THE UNCERTAINTY</span></Link>
        </div>
      </section>
      ...
      <section className="featured-section"><div className="section-heading-row"><div><span className="micro-label">FROM THE EVIDENCE TABLE</span><h2>Recently<br /><em>unsealed.</em></h2></div><Link href="/archive" onClick={navigateTo("/archive")} className="circle-arrow" aria-label="Browse all records"><ArrowUpRight size={23} /></Link></div><div className="feature-stage"><div className="feature-stamp"><span>EXAMINED</span><strong>OMN</strong><small>2026</small></div><div className="feature-tape" /><FeatureCard index={0} /><FeatureCard index={1} /><FeatureCard index={2} /><div className="feature-stage-note">04 / MORE RECORDS<br />WAITING BELOW</div></div></section>

      <section className="om-inbox-section"><div className="inbox-heading"><span className="micro-label">OM’S SCRIBAL INBOX</span><h2>Questions still<br />on the <em>table.</em></h2><p>New prompts, fragments, and routes for future records. They are not answers — they are a reason to keep the lamp on.</p></div><div className="inbox-notes"><Link href="/submit-theory" onClick={navigateTo("/submit-theory")} className="inbox-note-link"><article><Dna size={17} /><span className="micro-label">METAMORPH / PENDING</span><h3>Can a divine birth story be read as a map of social change?</h3><i>Filed 08 / OMN</i></article></Link><Link href="/submit-theory" onClick={navigateTo("/submit-theory")} className="inbox-note-link"><article><BrainCircuit size={17} /><span className="micro-label">REDACTED METHOD / PENDING</span><h3>Why does an unknown object so quickly become “lost technology”?</h3><i>Filed 11 / OMN</i></article></Link><Link href="/submit-theory" onClick={navigateTo("/submit-theory")} className="inbox-note-link"><article><Sparkles size={17} /><span className="micro-label">CELESTIAL / PENDING</span><h3>What did a star mean before we turned it into a map?</h3><i>Filed 12 / OMN</i></article></Link></div><div className="inbox-route"><MapPinned size={18} /> <Link href="/submit-theory" onClick={navigateTo("/submit-theory")}>Send Om a theory letter <ArrowUpRight size={16} /></Link></div></section>

      <section ref={signalsRef} className="signal-preview-section"><div aria-hidden="true" className="signal-parallax-glyph"><span>✦</span><span>✦</span><span>✦</span></div><div className="signal-preview-rule"><span>02</span><span>THE QUESTIONS THAT GATHER A CROWD</span></div><div className="signal-preview-layout"><div className="signal-preview-copy"><span className="micro-label">THE MYTHIC ANOMALY INDEX</span><h2>Not every<br />pattern is a <em>path.</em></h2><p>Some theories arrive with the force of a revelation: alien visitors, crop-circle maps, lost technologies, guardians, mutations, and star-built pyramids. The Anomaly Index keeps the imaginative leap and the evidentiary counterweight in the same room.</p><Link href="/investigations" onClick={navigateTo("/investigations")} className="ink-button">Open the signal board <ArrowUpRight size={17} /></Link></div><Link href="/investigations" onClick={navigateTo("/investigations")} className="signal-preview-plate"><motion.img className="home-parallax-image" style={reduceMotion ? {} : { y: signalPlateY }} src="/assets/kemet-crop-survey.jpg" alt="Abstract geometry in a desert survey" /><span className="plate-label">UNVERIFIED<br />LANDSCAPE SIGNAL</span><span className="plate-pin" /></Link><div className="signal-preview-calligraphy">“The shape<br />of a question<br />matters.”</div></div></section>

      <section className="om-note-section"><div className="tape tape-one" /><div className="om-note-title"><span className="micro-label">A NOTE FROM THE DESK</span><h2>Not answers.<br />Better <em>questions.</em></h2></div><blockquote>“I made this room for the moment a piece of history feels larger than its label.”<cite>— Om Nandurkar, keeper of the archive</cite></blockquote><Link href="/archive-room" onClick={navigateTo("/archive-room")} className="om-room-link"><BookOpen size={18} /> Enter Om’s archive room</Link></section>
      <PageFooter />
    </main>
  );
}
