...
export default function Article() {
  ...
  const article = localRecord ? { ...localRecord, image: localRecord.image || "/assets/om-specimen-field-station.png", tone: localRecord.presentation.palette === "ink" ? "night" as const : "paper" as const, sections: [{ label: "I. OM’S FIELD TEXT", heading: localRecord.title, paragraphs: localRecord.body.split(/\n{2,}/).filter(Boolean) }] } : fallback;
  ...
  return <main className={`article-page article-${article.tone} ${localRecord ? "article-local-record" : ""}`} style={previewStyle}><ReadingRail current={article.category.toUpperCase()} />
    <section className="article-hero"><Link href="/archive" className="article-back"><ArrowLeft size={16} /> All records</Link><div className="article-hero-image"><img src={article.image} alt="" /><div /></div>{localRecord && <><div className={`article-vector-trace vector-${localRecord.presentation.vector}`} /><div className={`article-hero-motif motif-${localRecord.presentation.motif}`}><img src="/assets/om-field-station-stickers.png" alt="" /></div></>}<div className="article-hero-copy"><p className="eyebrow-light">{article.eyebrow}</p><h1>{article.title}</h1><p>{article.excerpt}</p><div className="article-meta"><span>{article.date}</span><span>{article.readTime}</span><span>{localRecord ? "LOCAL DRAFT / OMN" : "FIELD RECORD / OMN"}</span></div></div></section>
